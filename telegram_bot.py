...
import \
    aiohttp
from aiohttp import \
    web  # Добавляем для HTTP-сервера
from monitoring_scanner import \
    Scanner
import \
    asyncpg
...
class BotState:
        ...
        while True:
            try:
                for user_id, _ in ALLOWED_USERS:
                    ...
                    logger.debug(
                        f"Keep-alive check for user_id={user_id}")
                await asyncio.sleep(
                    KEEP_ALIVE_INTERVAL)
            except Exception as e:
                logger.error(
                    f"Keep-alive error: {str(e)}")
                await asyncio.sleep(
                    KEEP_ALIVE_INTERVAL)
    ...
    def init_user_state(
        ...
        if user_id not in self.user_states:
            ...
            asyncio.create_task(
                self.load_or_set_default_levels(
                    user_id))
    ...
    async def check_access(
        ...
        logger.debug(
            f"Checking access for chat_id={chat_id}")
        if chat_id not in [
            user[
                0]
            for
            ...
            try:
                ...
                logger.warning(
                    f"Cannot notify chat_id={chat_id}: {e}")
            logger.warning(
                f"Access denied for chat_id={chat_id}")
            return False
    ...
    async def set_menu_button(
        ...
        try:
            ...
            logger.info(
                "Menu button set to 'commands'")
        except Exception as e:
            logger.error(
                f"Failed to set menu button: {e}")

    async def load_or_set_default_levels(
            self,
            user_id):
        try:
            levels = await load_levels(
                user_id)
            if levels is None:
            ...
            self.user_states[
                ...
                reverse=True)
            logger.info(
                f"Loaded levels for user_id={user_id}: {levels}")
        except Exception as e:
            logger.error(
                f"Error loading levels for user_id={user_id}: {str(e)}, setting to empty list")
            self.user_states[
                user_id][
                'current_levels'] = []
            await save_levels(
    ...
    async def save_levels(
        ...
        try:
            ...
            self.user_states[
                ...
                'current_levels'] = levels
            logger.debug(
                f"Saved levels for user_id={user_id}: {levels}")
        except Exception as e:
            logger.error(
                f"Error saving levels for user_id={user_id}: {str(e)}")
    ...
    async def load_user_stats(
        ...
        try:
            ...
            logger.error(
                f"Error loading stats for user_id={user_id}: {str(e)}")
            self.init_user_stats(
                user_id)
        ...
        try:
            await save_stats(
                ...
                self.user_stats[
                    user_id])
            logger.debug(
                f"Saved stats for user_id={user_id}")
        except Exception as e:
            logger.error(
                f"Error saving stats for user_id={user_id}: {str(e)}")
    ...
    async def update_message(
        ...
        try:
            ...
            logger.debug(
                f"Sent new message_id={msg.message_id} for chat_id={chat_id}")
        except Exception as e:
            logger.error(
                f"Error sending message to chat_id={chat_id}: {str(e)}")

    async def confirm_level_crossing(
            self,
            chat_id,
            initial_value,
            direction,
        ...
        state[
            'direction'] = direction
        logger.info(
            f"Starting confirmation for chat_id={chat_id}: {initial_value:.6f} Gwei, direction: {direction}, target: {target_level:.6f}")
        ...
        for i in range(
            ...
            if current_slow is None:
                logger.error(
                    f"Failed to get gas on attempt {i + 2} for chat_id={chat_id}")
                state[
                    'count'] = 0
                state[
                    'values'] = []
            ...
            state[
                ...
                current_slow)
            logger.debug(
                f"Attempt {i + 2} for chat_id={chat_id}: {current_slow:.6f} Gwei")

        values = \
        state[
            'values']
        is_confirmed = False
        if direction == 'down' and all(
                v <= target_level
                for
                v
                in
        ...
        if is_confirmed and target_level not in \
                self.user_states[
                    chat_id][
                    'notified_levels']:
            last_measured = \
            ...
            self.user_states[
                chat_id][
                'prev_level'] = last_measured
            logger.info(
                f"Level {target_level:.6f} confirmed for chat_id={chat_id}, notified")
    ...
    async def get_manta_gas(
            self,
            chat_id,
            force_base_message=False):
        try:
            current_slow = await self.scanner.get_current_gas()
            if current_slow is None:
                await self.update_message(
                    chat_id,
                    "<b>⚠️ Не удалось подключиться к Manta Pacific</b>",
                    create_main_keyboard(
                        chat_id))
                return

            logger.info(
                f"Gas for chat_id={chat_id}: Slow={current_slow:.6f}")
            base_message = f"<pre>⛽️ Manta Pacific Gas\n◆ <b>ТЕКУЩИЙ ГАЗ</b>:   {current_slow:.6f} Gwei</pre>"

            self.user_states[
                chat_id][
                'last_measured_gas'] = current_slow
            prev_level = \
            self.user_states[
                chat_id][
                'prev_level']
            ...
            if not levels:
                await self.load_or_set_default_levels(
                    chat_id)
                levels = \
                self.user_states[
                    chat_id][
            ...
            if not levels:
                if force_base_message:
                    await self.update_message(
                        chat_id,
                        base_message + "\n\nУровни не заданы. Используйте 'Задать уровни'.",
                        create_main_keyboard(
                            chat_id))
                else:
                    logger.info(
                        f"No levels set for chat_id={chat_id}, skipping notification check.")
                self.user_states[
                    chat_id][
                    'prev_level'] = current_slow
                return
            ...
            confirmation_state[
                ...
                for level in sorted_levels:
                    if prev_level < level <= current_slow:
                        logger.info(
                            f"Detected upward crossing for chat_id={chat_id}: {level:.6f}")
                        asyncio.create_task(
                            self.confirm_level_crossing(
                                chat_id,
                                current_slow,
                                'up',
                                level))
                        ...
                        logger.info(
                            f"Detected downward crossing for chat_id={chat_id}: {level:.6f}")
                        asyncio.create_task(
                            self.confirm_level_crossing(
                                chat_id,
                                current_slow,
            ...
            logger.error(
                f"Error for chat_id={chat_id}: {str(e)}")
            await self.update_message(
                chat_id,
                f"<b>⚠️ Ошибка:</b> {str(e)}",
                create_main_keyboard(
                    chat_id))
    ...
    async def fetch_l2_data(
        ...
        if self.l2_data_time and (
                current_time - self.l2_data_time).total_seconds() < self.l2_data_cooldown and self.l2_data_cache:
                    ...
                    if coin:
                        ...
                        price_change_all = (
                                    (
                                            ...
                                            price)) / coin.get(
                                "ath",
                                price) * 100) if price != "Н/Д" and coin.get(
                            "ath") else "Н/Д"
                        ath_price = coin.get(
                            "ath",
                            "Н/Д")
    ...
    async def fetch_fear_greed(
        ...
        if self.fear_greed_time and (
                current_time - self.fear_greed_time).total_seconds() < self.fear_greed_cooldown and self.fear_greed_cache:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                ...
                if response.status != 200:
                    logger.error(
                        f"CMC Fear & Greed API error: {response.status}")
                    return None
                data = await response.json()
                if "data" not in data or not \
                data[
                    "data"]:
                    logger.error(
                ...
                max_year_date = parse_timestamp(
                    max_year_date)
                min_year_date = parse_timestamp(
                    min_year_date)
                ...
                fear_greed_data = {
                    ...
                    "yesterday": {
                        ...
                        "category": yesterday_category},
                    "week_ago": {
                        "value": week_ago_value,
                        "category": week_ago_category},
                    "month_ago": {
                        "value": month_ago_value,
                        "category": month_ago_category},
                    "year_max": {
                        "value": max_year_value,
                        "date": max_year_date,
                        "category": max_year_category},
                    "year_min": {
                        "value": min_year_value,
                        "date": min_year_date,
                        "category": min_year_category}
                }
    ...
    async def get_manta_price(
        ...
        try:
            ...
            if not token_data:
                await self.update_message(
                    chat_id,
                    "⚠️ Не удалось получить данные от CoinGecko.",
                    create_main_keyboard(
                        chat_id))
                return
            ...
            spot_volume_m = round(
                spot_volume / Decimal(
                    '1000000')) if spot_volume else 0
            futures_volume_m = round(
                futures_volume / Decimal(
                    '1000000')) if futures_volume else 0
            ...
            logger.error(
                f"Error fetching price for chat_id={chat_id}: {str(e)}")
            await self.update_message(
                chat_id,
                f"<b>⚠️ Ошибка:</b> {str(e)}",
                create_main_keyboard(
    ...
    async def get_l2_comparison(
        ...
        try:
            ...
            for name, data in sorted_by_24h:
                price_str = f"${float(data['price']):.3f}" if \
                data[
                    'price'] not in (
                "Н/Д",
                None) else "Н/Д"
                change_str = f"{float(data['24h']):>6.2f}%" if \
                data[
                    '24h'] not in (
                "Н/Д",
                None) else "Н/Д"
            ...
            for name, data in sorted_by_7d:
                ...
                "Н/Д",
                None) else "Н/Д"
                change_str = f"{float(data['7d']):>6.2f}%" if \
                data[
                    '7d'] not in (
                "Н/Д",
                None) else "Н/Д"
            ...
            for name, data in sorted_by_30d:
                ...
                "Н/Д",
                None) else "Н/Д"
                change_str = f"{float(data['30d']):>6.2f}%" if \
                data[
                    '30d'] not in (
                "Н/Д",
                None) else "Н/Д"
            ...
            for name, data in sorted_by_all:
                ...
                "Н/Д",
                None) else "Н/Д"
                change_str = f"{float(data['all']):>6.2f}%" if \
                data[
                    'all'] not in (
                "Н/Д",
                None) else "Н/Д"
            ...
            logger.error(
                f"Error fetching L2 comparison for chat_id={chat_id}: {str(e)}")
            await self.update_message(
                chat_id,
                f"<b>⚠️ Ошибка:</b> {str(e)}",
                create_main_keyboard(
    ...
    async def get_fear_greed(
        ...
        try:
            ...
            if not fg_data:
                await self.update_message(
                    chat_id,
                    "⚠️ Не удалось получить данные Fear & Greed от CoinMarketCap.",
                    create_main_keyboard(
                        chat_id))
                return
            ...
            logger.error(
                f"Error fetching Fear & Greed for chat_id={chat_id}: {str(e)}")
            await self.update_message(
                chat_id,
                f"<b>⚠️ Ошибка:</b> {str(e)}",
                create_main_keyboard(
...
def create_delete_levels_keyboard(
        levels):
    keyboard = [
        [
            types.KeyboardButton(
                text=f"Удалить {level:.4f} Gwei")]
        for
        level
        in
        levels]
...
async def start_command(
        message: types.Message):
    if not await state.check_access(
            message):
        return
    chat_id = message.chat.id
    logger.info(
        f"Start command received from chat_id={chat_id}")
    await state.update_message(
        chat_id,
        "<b>Бот для Manta Pacific запущен.</b>\nВыберите действие:",
        create_main_keyboard(
            chat_id))
    try:
        ...
        logger.error(
            f"Failed to delete start command message_id={message.message_id}: {e}")
...
async def handle_main_button(
    ...
    text = message.text
    logger.debug(
        f"Button pressed: {text} by chat_id={chat_id}")
    ...
    if text == "Проверить газ":
        await state.get_manta_gas(
            chat_id,
            force_base_message=True)
    elif text == "Manta Price":
        await state.get_manta_price(
            chat_id)
    elif text == "Сравнение L2":
        await state.get_l2_comparison(
            chat_id)
    elif text == "Страх и Жадность":
        await state.get_fear_greed(
            chat_id)
    elif text == "Задать уровни":
        state.pending_commands[
            chat_id] = {
            'step': 'range_selection'}
        await state.update_message(
            chat_id,
            "Выберите действие для уровней уведомлений:",
            create_levels_menu_keyboard())
    ...
    try:
        ...
        logger.error(
            f"Failed to delete user message_id={message.message_id}: {e}")
    ...
    if chat_id not in state.pending_commands:
        ...
        state_data[
            ...
            if text == "Отмена":
                ...
                await state.update_message(
                    chat_id,
                    "Введите уровень от 0,0001 до 0,01:",
                    create_level_input_keyboard())
                ...
                state.user_states[
                    ...
                    state_data[
                        'step'] = 'delete_level_selection'
                    await state.update_message(
                        chat_id,
                        "Выберите уровень для удаления:",
                        create_delete_levels_keyboard(
                            state.user_states[
                                chat_id][
                                'current_levels']))
            else:
        ...
        state_data[
            ...
            if text == "Отмена":
                ...
                await state.update_message(
                    chat_id,
                    "Возврат к выбору диапазона.",
                    create_levels_menu_keyboard())
            elif text == "Добавить еще уровень" or text == "Завершить":
                await state.update_message(
                    chat_id,
                    "Сначала введите уровень.",
                    create_level_input_keyboard())
            else:
                try:
                    ...
                    if not (
                        ...
                        await state.update_message(
                            chat_id,
                            f"Ошибка: введите значение в диапазоне {min_val}–{max_val}",
                            create_level_input_keyboard())
                        return
                    ...
                    if len(
                        ...
                        await state.update_message(
                            chat_id,
                            "Достигнут лимит в 100 уровней. Уровни сохранены.",
                            create_main_keyboard(
                                chat_id))
                    else:
                        state_data[
                        ...
                        await state.update_message(
                            chat_id,
                            f"Уровень {level:.6f} добавлен. Что дальше?",
                            create_level_input_keyboard())
                except ValueError:
                    await state.update_message(
                        chat_id,
        ...
        state_data[
            ...
            if text == "Отмена":
                ...
                await state.update_message(
                    chat_id,
                    f"Введите уровень от {min_val} до {max_val}:",
                    create_level_input_keyboard())
                ...
                await state.update_message(
                    chat_id,
                    f"Введите следующий уровень (в пределах {min_val}–{max_val}):",
                    create_level_input_keyboard())
            elif text == "Завершить":
                await state.save_levels(
                    chat_id,
        ...
        state_data[
            ...
            if text == "Отмена":
                ...
                await state.update_message(
                    ...
                    "Удалить "):
                level_str = text.replace(
                    "Удалить ",
                    "").replace(
                    " Gwei",
                    "")
                try:
                    level_to_delete = Decimal(
                        level_str)
                    if level_to_delete in \
                            state.user_states[
                                chat_id][
                                'current_levels']:
                        state.user_states[
                            chat_id][
                            'current_levels'].remove(
                            level_to_delete)
                        await state.save_levels(
                            chat_id,
                            state.user_states[
                        ...
                        await state.update_message(
                            chat_id,
                            f"Уровень {level_to_delete:.6f} Gwei удалён.",
                            create_main_keyboard(
                                chat_id))
                    else:
                        await state.update_message(
...
async def main():
    ...
    for user_id, _ in ALLOWED_USERS:
        ...
        state.init_user_stats(
            user_id)
        await state.load_or_set_default_levels(
            user_id)
        await state.load_user_stats(
            user_id)
